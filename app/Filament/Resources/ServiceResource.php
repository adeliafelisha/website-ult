...
class ServiceResource extends Resource
{
    ...
    public static function form(Form $form): Form
    {
        return $form->schema([
            ...
            Forms\Components\Section::make('Tombol kontak')->description('Minimal satu tombol harus tersedia. Tombol pertama secara bawaan mengarah ke WhatsApp Admin ULT, tetapi label, kanal, URL, dan urutannya dapat diubah.')->schema([
                Forms\Components\Repeater::make('contact_buttons')->label('Daftar tombol')->schema([
                    Forms\Components\TextInput::make('label')->label('Tulisan tombol (Indonesia)')->default('Hubungi Admin ULT')->required(),
                    Forms\Components\TextInput::make('label_en')->label('Button text (English)')->default('Contact ULT Admin'),
                    Forms\Components\Select::make('channel')->label('Kanal')->options(['whatsapp' => 'WhatsApp', 'helpdesk' => 'Helpdesk', 'email' => 'Email', 'phone' => 'Telepon', 'website' => 'Website/sistem', 'other' => 'Lainnya'])->default('whatsapp')->required(),
                    Forms\Components\TextInput::make('url')->label('Link tujuan')->default(fn (): string => Contact::where('type', 'whatsapp')->where('is_published', true)->orderBy('sort_order')->value('url') ?: 'https://wa.me/6280000000000')->url()->required(),
                ])->columns(2)->defaultItems(1)->minItems(1)->reorderable()->collapsible()->columnSpanFull(),
            ]),
            Forms\Components\Section::make('Media & publikasi')->schema([Forms\Components\FileUpload::make('featured_image')->image()->acceptedFileTypes(['image/jpeg', 'image/png', 'image/webp'])->maxSize(5120)->directory('services'), Forms\Components\TagsInput::make('keywords')->separator(','), Forms\Components\Toggle::make('is_featured')->label('Unggulan'), Forms\Components\Toggle::make('is_published')->label('Terbit'), Forms\Components\DateTimePicker::make('published_at')->label('Waktu terbit')])->columns(2),
        ]);
    }

    public static function table(Table $table): Table
    {
        return $table->columns([Tables\Columns\TextColumn::make('title')->searchable()->sortable(), Tables\Columns\TextColumn::make('category.name')->label('Kategori'), Tables\Columns\TextColumn::make('delivery_type')->badge(), Tables\Columns\IconColumn::make('cta_url')->label('Link')->boolean(), Tables\Columns\IconColumn::make('is_published')->label('Terbit')->boolean(), Tables\Columns\TextColumn::make('updated_at')->label('Diperbarui')->since()])->filters([Tables\Filters\TernaryFilter::make('is_published')->label('Status terbit')])->actions([Tables\Actions\EditAction::make(), Tables\Actions\DeleteAction::make()])->bulkActions([Tables\Actions\BulkAction::make('publish')->label('Terbitkan')->icon('heroicon-o-eye')->action(fn (Collection $records) => $records->each->update(['is_published' => true, 'published_at' => now()]))->deselectRecordsAfterCompletion(), Tables\Actions\BulkAction::make('unpublish')->label('Batalkan terbit')->icon('heroicon-o-eye-slash')->color('warning')->action(fn (Collection $records) => $records->each->update(['is_published' => false]))->deselectRecordsAfterCompletion(), Tables\Actions\DeleteBulkAction::make()])->defaultSort('updated_at', 'desc');
    }
