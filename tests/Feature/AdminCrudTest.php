...
class AdminCrudTest extends TestCase
{
    ...
    public function test_verified_admin_can_open_all_crud_pages(): void
    {
        ...
        foreach (['/admin', '/admin/services', '/admin/service-categories', '/admin/articles', '/admin/faqs', '/admin/contacts', '/admin/quick-links'] as $url) {
    ...
    public function test_cms_changes_are_immediately_visible_in_both_languages(): void
    {
        ...
        $service = Service::create(['service_category_id' => $category->id, 'title' => 'Layanan Baru', 'title_en' => 'New Service', 'slug' => 'layanan-baru', 'summary' => 'Ringkasan baru', 'summary_en' => 'New summary', 'delivery_type' => 'online', 'cta_label' => 'Buka', 'cta_label_en' => 'Open', 'is_published' => true, 'published_at' => now()]);
        $this->get('/language/id');
        $this->get('/layanan/'.$service->slug)->assertSee('Layanan Baru');
        $this->get('/language/en');
        $this->get('/layanan/'.$service->slug)->assertSee('New Service')->assertSee('New summary');
